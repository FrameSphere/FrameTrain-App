...
def _audio_in(d: Path) -> List[Path]:
    return sorted(f for f in d.rglob("*") if f.suffix.lower() in AUDIO_EXTS)
    ...
    def load_data(self) -> None:
        ...
        if len(self.classes) < 2:
            raise ValueError(
                ...
                f"({self.classes or 'keine'}). Fuer eine Audioklassifikation braucht es "
        ...
        if not train_raw["path"]:
            raise ValueError(
                f"Keine Audiodateien unter '{train_root}' gefunden. "
                f"Erwartet werden: {', '.join(sorted(AUDIO_EXTS))}"
            )
        MessageProtocol.status(
            "loading_data",
            f"Klassen ({len(self.classes)}): {', '.join(self.classes)} | "
            f"{len(train_raw['path'])} Trainingsdateien",
        )
